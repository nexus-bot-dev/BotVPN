   ...
   return new Promise((resolve, reject) => {
     ...
     if (notificationSent) {
       ...
       try {
        ...
        // Jika top up Rp 5.000, berikan bonus Rp 3.000 otomatis
        try {
          const original = Number(deposit.originalAmount);
          if (original === 5000) {
            const bonus = 3000;
            await new Promise((resolve, reject) => {
              db.run('UPDATE users SET saldo = saldo + ? WHERE user_id = ?', [bonus, deposit.userId], function(err) {
                if (err) {
                  logger.error('Gagal menambahkan bonus topup:', err.message);
                  return reject(err);
                }
                resolve();
              });
            ...
            // Perbarui nilai saldo yang sudah diambil sebelumnya agar notif menampilkan saldo akhir yang benar
            if (user && typeof user.saldo === 'number') {
              user.saldo = Number(user.saldo) + bonus;
            }

            // Kirim notifikasi singkat ke user tentang bonus
            try {
              await bot.telegram.sendMessage(deposit.userId, `🎁 Bonus Top Up!\n\nTerima kasih telah melakukan Top Up Rp ${deposit.originalAmount}. Anda mendapat bonus Rp ${bonus} yang sudah otomatis ditambahkan ke saldo Anda.`, { parse_mode: 'Markdown' });
            } catch (e) {
              logger.error('Gagal mengirim notifikasi bonus ke user:', e.message);
            }
          }
        } catch (e) {
          logger.error('Error saat memproses bonus topup:', e.message);
        }userInfo = await bot.telegram.getChat(deposit ? deposit.userId : (ctx ? ctx.from.id : ''));
         } catch (e) {
           userInfo = {};
         }
